...
//++++++++++++++++ added by alkaid+++++++++++++++++++++++
    function step_3(){
        $model = $this->getModel('member_public_token_io');
        $id = I ( 'get.id' );
		$this->assign ( 'id', $id );
		$data=M('member_public_token_io')->where('mp_id='.$id)->find();
//        $id=$data['id'];

        if (IS_POST) {
			$mpti_id = I ( 'post.id', 0, 'intval' );
			$_POST['mp_id']=$id;
            foreach ( $_POST as &$v ) {
                $v = trim ( $v );
            }
			...
			if (empty ( $mpti_id )) {
				if ($Model->create () && $id = $Model->add ()) {
					$this->success ( '添加基本信息成功！', U ( 'lists?model=' . $this->model ['name'] ) );
				} else {
					$this->error ( $Model->getError () );
				}
			} else {
				...
				$Model->create () && $res = $Model->save ();
				if ($res) {
					$this->success ( '保存基本信息成功！', U ( 'lists?model=' . $this->model ['name'] ) );
				} elseif ($res === 0) {
					$this->success ( ' ', U ( 'lists?model=' . $this->model ['name'] ) );
				} else {
					$this->error ( $Model->getError () );
				}
			}
        } else {
			$data ['input_switch'] = intval ( $data ['input_switch'] );
			$data ['output_switch'] = intval ( $data ['output_switch'] );
            $this->assign ( 'info', $data );
            $this->display ( 'step_3' );
        }
    }
